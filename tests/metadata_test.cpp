...
std::shared_ptr<cpptoml::table> create_metadata_config(
...
go_bandit([]() {
  describe("[metadata]", []() {
    ...
    using options_type = const std::vector<std::pair<std::string, std::string>>;
    ...
    it("should read string metadata with spaces", [&]() {
      ...
      AssertThat(fields[0].type, Equals(corpus::metadata::field_type::STRING));
      AssertThat(fields[0].str, Equals("/my/path1"));
      AssertThat(fields[1].type, Equals(corpus::metadata::field_type::STRING));
      AssertThat(fields[1].str, Equals("Wonderful Ducklings"));
      AssertThat(fields[2].type, Equals(corpus::metadata::field_type::STRING));
