...
go_bandit([]() {
  describe("[ir-eval] retrieval metrics", []() {
    it("should give results on [0, 1] for all measures", []() {
      ...
      for (size_t i = 0; i < 5; ++i) {
        ...
        AssertThat(f1, Is().GreaterThanOrEqualTo(0).And().LessThanOrEqualTo(1));
        AssertThat(p, Is().GreaterThanOrEqualTo(0).And().LessThanOrEqualTo(1));
        AssertThat(r, Is().GreaterThanOrEqualTo(0).And().LessThanOrEqualTo(1));
        AssertThat(avg_p,
    ...
    it("should compute correct eval measures", []() {
      ...
      check_query(eval, results, qid, 0.2 / 1.1, 1, 0.1, 0.2, 1.0 / idcg_5, 5);

      results.emplace_back(doc_id{2}, 0.9);  // not relevant
      check_query(eval, results, qid, 0.1 / 0.6, 0.5, 0.1, 0.1, 1.0 / idcg);
      check_query(eval, results, qid, 0.1 / 0.6, 0.5, 0.1, 0.2, 1.0 / idcg_5,
                  5);
      results.emplace_back(doc_id{1}, 0.8);  // relevant
      check_query(eval, results, qid,
                  (2.0 * (2.0 / 3.0) * 0.2) / (2.0 / 3.0 + 0.2), 2.0 / 3.0, 0.2,
                  1.0 / 6.0, 1.5 / idcg);
      check_query(eval, results, qid,
                  (2.0 * (2.0 / 3.0) * 0.2) / (2.0 / 3.0 + 0.2), 2.0 / 3.0, 0.2,
                  1.0 / 3.0, 1.5 / idcg_5, 5);
      ...
      auto avg_p_5 = (1.0 + 2.0 / 3.0 + 3.0 / 4.0 + 4.0 / 5.0) / 5.0;
      auto avg_p =
          (1.0 + 2.0 / 3.0 + 3.0 / 4.0 + 4.0 / 5.0 + 5.0 / 6.0 + 6.0 / 7.0 +
      ...
      check_query(eval, results, qid,
                  (2.0 * (10.0 / 11.0)) / ((10.0 / 11.0) + 1.0), 10.0 / 11.0,
                  1.0, avg_p, dcg / idcg);
      check_query(eval, results, qid,
                  (2.0 * (4.0 / 5.0) * 0.4) / ((4.0 / 5.0) + 0.4), 4.0 / 5.0,
                  0.4, avg_p_5, dcg_5 / idcg_5, 5);
  ...
  describe("[ir-eval] rank correlation metrics", []() {
    ...
    it("should throw on nonequal list sizes", []() {
      ...
      auto bad_func = [&]() { index::rank_correlation corr{rank_x, rank_y}; };
