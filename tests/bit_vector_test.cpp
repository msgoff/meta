...
go_bandit([]() {
  ...
  describe("[bit vector]", []() {
    ...
    it("should build files of the correct size", [&]() {
      ...
      AssertThat(filesystem::file_size(filename), Equals(sizeof(uint64_t) * 2));
      filesystem::delete_file(filename);
    });

    it("should correctly extract single bits", [&]() {
      auto alternating_ones = static_cast<uint64_t>(0xaaaaaaaaaaaaaaaaULL);
      ...
      uint64_t total_size = std::accumulate(sizes.begin(), sizes.end(), 0ull);
      {
        ...
        for (const auto& size : sizes)
          builder.write_bits({alternating_ones, static_cast<uint8_t>(size)});
      ...
      AssertThat(
          ...
          Equals(sizeof(uint64_t) * math::integer::div_ceil(total_size, 64)));

      {
        util::disk_vector<uint64_t> storage{filename};
        util::array_view<const uint64_t> av{storage.begin(), storage.end()};
    ...
    it("should correctly extract multi-bit patterns", [&]() {
          ...
          Equals(sizeof(uint64_t) * math::integer::div_ceil(total_size, 64)));
      {
        ...
        for (const auto& sze : sizes) {
          ...
          auto mask =
              size == 64 ? static_cast<uint64_t>(-1) : (1ull << size) - 1;
