...
go_bandit([]() {
  ...
  describe("[perfect hash]", []() {
    it("should generate minimum perfect hash functions on strings", []() {
      ...
      options.num_keys = filesystem::num_lines("../data/lemur-stopwords.txt");
...

      {
        ...
        std::string line;
        while (std::getline(input, line)) builder(line);

        builder.write();
      }

      {
        hashing::perfect_hash<std::string> mph{"perfect-hash-unit-test"};
    ...
    it("should generate perfect hash functions on vectors of ints", []() {
      using mph_builder = hashing::perfect_hash_builder<std::vector<uint64_t>>;
      using options_type = mph_builder::options;
      filesystem::remove_all("perfect-hash-unit-test");

      std::vector<std::vector<uint64_t>> keys = {{1, 2, 3},
                                                 {4, 5, 6},
      ...
      options.max_ram = 1024 * 1024;  // 1MB
      {
        ...
        for (const auto& vec : keys) builder(vec);

        builder.write();
      }

      {
        hashing::perfect_hash<std::vector<uint64_t>> mph{options.prefix};
