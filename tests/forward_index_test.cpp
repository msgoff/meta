...
void check_ceeaus_expected_fwd(Index& idx) {
  ...
  AssertThat(label_ids.find(label_id{1}), Is().Not().EqualTo(label_ids.end()));
  AssertThat(label_ids.find(label_id{2}), Is().Not().EqualTo(label_ids.end()));
  AssertThat(label_ids.find(label_id{3}), Is().Not().EqualTo(label_ids.end()));
