...
void whitespace_tokenizer::set_content(std::string&& content) {
  ...
  it_ = content_.begin();
  if (suppress_whitespace_) consume_adjacent_whitespace();
}

void whitespace_tokenizer::consume_adjacent_whitespace() {
  it_ = std::find_if_not(it_, content_.cend(),
                         [](char c) { return std::isspace(c); });
}

std::string whitespace_tokenizer::next() {
  if (!*this) throw token_stream_exception{"next() called with no tokens left"};
  ...
  auto begin = it_;
  it_ = std::find_if(it_, content_.cend(),
                     [](char c) { return std::isspace(c); });
...
template <>
std::unique_ptr<token_stream> make_tokenizer<whitespace_tokenizer>(
