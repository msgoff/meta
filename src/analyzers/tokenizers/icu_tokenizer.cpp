 ...
 public:
  ...
  void set_content(std::string content) {
    ...
    for (const auto& sentence : segmenter_.sentences()) {
      if (!suppress_tags_) tokens_.emplace_back("<s>");
      for (const auto& word : segmenter_.words(sentence)) {
        ...
        if (codepoint < 0 || utf::isspace(static_cast<uint32_t>(codepoint)))
          continue;
   ...
   */
  explicit operator bool() const { return !tokens_.empty(); }
...
std::string icu_tokenizer::next() { return impl_->next(); }

icu_tokenizer::operator bool() const { return static_cast<bool>(*impl_); }

template <>
std::unique_ptr<token_stream> make_tokenizer<icu_tokenizer>(
