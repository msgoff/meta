...
std::string get_content(const corpus::document& doc) {
  if (!doc.contains_content())
    throw analyzer_exception{"document content was not populated for analysis"};

  return utf::to_utf8(doc.content(), doc.encoding());
}

namespace {
std::unique_ptr<token_stream> add_default_filters(
    std::unique_ptr<token_stream> tokenizer, const cpptoml::table& config) {
...
std::unique_ptr<token_stream> default_filter_chain(
...
std::unique_ptr<token_stream> default_unigram_chain(
    const cpptoml::table& config) {
  // suppress "<s>", "</s>"
...
std::unique_ptr<token_stream> load_filter(std::unique_ptr<token_stream> src,
  ...
  if (!type) throw analyzer_exception{"filter type missing in config file"};
...
std::unique_ptr<analyzer> load(const cpptoml::table& config) {
  ...
  for (auto group : analyzers->get()) {
    ...
    if (!method) throw analyzer_exception{"failed to find analyzer method"};
    toks.emplace_back(analyzer_factory::get().create(*method, config, *group));
  }
