...
void sentence_boundary::load_heuristics(const cpptoml::table& config) {
  ...
  if (!start_exceptions)
    throw token_stream_exception{"configuration missing start exceptions file"};

  auto end_exceptions = config.get_as<std::string>("end-exceptions");
  if (!end_exceptions)
    throw token_stream_exception{"configuration missing end-exceptions file"};

  if (!filesystem::file_exists(*punc))
    throw token_stream_exception{"punctuation file does not exist: " + *punc};

  std::ifstream punc_file{*punc};
  std::string line;
  while (std::getline(punc_file, line)) punc_set.emplace(std::move(line));
...
std::unique_ptr<token_stream> make_filter<sentence_boundary>(
    std::unique_ptr<token_stream> src, const cpptoml::table& config) {
