...
list_filter::list_filter(std::unique_ptr<token_stream> source,
  ...
  if (!file) throw token_stream_exception{"invalid file for list filter"};

  std::string line;
  while (std::getline(file, line)) list_.emplace(std::move(line));
...
list_filter::operator bool() const { return token_ || *source_; }
