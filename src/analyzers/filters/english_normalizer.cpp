...
std::string english_normalizer::next() {
  ...
  if (!tokens_.empty()) return current_token();
  ...
  if (is_whitespace(token)) {
    while (is_whitespace(token) && *source_) token = source_->next();
...
void english_normalizer::parse_token(const std::string& token) {
  ...
  while (idx < end) idx = word(idx, token);

  if (end_quotes) tokens_.push_back("''");
}
...
bool english_normalizer::is_quote(char c) { return c == '\'' || c == '`'; }
...
uint64_t english_normalizer::word(uint64_t start, const std::string& token) {
  ...
  while (idx < token.length()) {
    ...
    if (std::ispunct(token[idx]) && token[idx] != '-') break;
    ++idx;
  }
