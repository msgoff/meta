...
word_embeddings::word_embeddings(std::istream& vocab, std::istream& vectors)
  ...
  for (std::size_t tid = 0; tid < id_to_term_.size() + 1; ++tid) {
    if (!vectors)
      throw word_embeddings_exception{"embeddings stream ended unexpectedly"};
...
util::string_view word_embeddings::term(std::size_t tid) const {
  if (tid >= id_to_term_.size()) return "<unk>";
  return id_to_term_[tid];
}

std::vector<scored_embedding> word_embeddings::top_k(
    util::array_view<const double> query, std::size_t k) const {
...
std::size_t word_embeddings::vector_size() const { return vector_size_; }

word_embeddings load_embeddings(const cpptoml::table& config) {
  auto prefix = config.get_as<std::string>("prefix");
  if (!prefix)
    throw word_embeddings_exception{"missing prefix key in configuration file"};
  ...
  if (!vocab)
    throw word_embeddings_exception{"missing vocabulary file in: " + *prefix};

  std::ifstream target{*prefix + "/embeddings.target.bin", std::ios::binary};
  std::ifstream context{*prefix + "/embeddings.context.bin", std::ios::binary};

  auto mode = config.get_as<std::string>("mode").value_or("average");
  if (mode == "average") {
    if (!target)
      throw word_embeddings_exception{"missing target vectors in: " + *prefix};
    if (!context)
      throw word_embeddings_exception{"missing context vectors in: " + *prefix};
