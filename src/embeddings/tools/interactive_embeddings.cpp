...
void parse_whitespace(util::string_view& query) {
  while (!query.empty() && std::isspace(query.front())) query = query.substr(1);
}

util::array_view<const double> parse_word(
    util::string_view& query, const embeddings::word_embeddings& glove) {
  ...
  if (word.empty()) throw parse_exception{"invalid expression"};
  parse_whitespace(query);
...
std::vector<double> parse_expression(util::string_view& query,
  ...
  if (query.empty()) throw parse_exception{"invalid expression"};
