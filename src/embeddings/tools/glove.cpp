...
std::size_t shuffle_partition(const std::string& prefix, std::size_t max_ram,
  ...
  auto elapsed = common::time([&]() {
    printing::progress progress{" > Shuffling (pass 1): ", input.total_bytes()};
    while (input != cooccur_iterator{}) {
      std::size_t i = 0;
      for (; i < records.size() && input != cooccur_iterator{}; ++i, ++input) {
        progress(input.bytes_read());
        records[i] = *input;
      }

      std::shuffle(records.begin(), records.begin() + static_cast<diff_type>(i),
                   engine);

      std::ofstream output{prefix + "/cooccur-shuf." +
                               std::to_string(chunk_sizes.size()) + ".tmp",
                           std::ios::binary};

      total_records += i;
      chunk_sizes.push_back(i);
      for (std::size_t j = 0; j < i; ++j) io::packed::write(output, records[j]);
    }
  });
  ...
  for (std::size_t i = 0; i < chunk_sizes.size(); ++i) {
    chunks.emplace_back(prefix + "/cooccur-shuf." + std::to_string(i) + ".tmp");
  }
    ...
    while (true) {
      ...
      for (std::size_t j = 0; j < chunk_sizes.size(); ++j) {
        ...
        for (std::size_t n = 0; n < to_write; ++n) {
          if (chunks[j] == cooccur_iterator{} || i == records.size()) break;
          records[i] = *chunks[j];
          ++chunks[j];
          ++i;
          progress(++num_read);
...
class glove_trainer {
 public:
  glove_trainer(const cpptoml::table& embed_cfg) {
    ...
    auto max_ram =
        embed_cfg.get_as<std::size_t>("max-ram").value_or(4096) * 1024 * 1024;
    vector_size_ = embed_cfg.get_as<std::size_t>("vector-size").value_or(50);
    ...
    learning_rate_ = embed_cfg.get_as<double>("learning-rate").value_or(0.05);
    ...
    if (!filesystem::file_exists(prefix + "/cooccur.bin")) {
      LOG(fatal) << "Cooccurrence matrix has not yet been generated, please "
                    "do this before learning word embeddings"
                 << ENDLG;
 ...
 private:
  ...
  double& context_bias_gradsq(uint64_t term) {
    return grad_squared_[(term * 2 + 1) * (vector_size_ + 1) + vector_size_];
  }
    ...
    for (std::size_t i = 1; i <= iters; ++i) {
      ...
      for (std::size_t t = 0; t < num_threads; ++t) {
        futures.emplace_back(pool.submit_task(
            [&, t]() { return train_thread(prefix, t, progress, records); }));
      }

      double total_cost = 0.0;
      auto elapsed = common::time([&]() {
        for (auto& fut : futures) total_cost += fut.get();
      });
      progress.end();
  ...
  double train_thread(const std::string& prefix, std::size_t thread_id,
    ...
    for (; iter != cooccur_iterator{}; ++iter) {
      ...
      auto diff =
          score(record.target, record.context) - std::log(record.weight);
  ...
  void save(const std::string& prefix, uint64_t num_words,
    ...
    // target embeddings
    {
      std::ofstream output{prefix + "/embeddings.target.bin", std::ios::binary};
      printing::progress progress{" > Saving target embeddings: ", num_words};
    ...
    // context embeddings
    {
      std::ofstream output{prefix + "/embeddings.context.bin",
                           std::ios::binary};
      printing::progress progress{" > Saving context embeddings: ", num_words};
      io::packed::write(output, vector_size_);
      save_embeddings(output, num_words, num_rare, progress,
  ...
  void save_embeddings(std::ofstream& output, uint64_t num_words,
    ...
    for (uint64_t tid = num_words - num_rare; tid < num_words; ++tid) {
      ...
      std::transform(unk_vec.begin(), unk_vec.end(), vec.begin(),
                     unk_vec.begin(), [=](double unkweight, double vecweight) {
  ...
  void write_normalized(ForwardIterator begin, ForwardIterator end,
    ...
    auto len = std::sqrt(std::accumulate(
        begin, end, 0.0,
        [](double accum, double weight) { return accum + weight * weight; }));
