...
class cooccurrence_buffer {
 public:
  ...
  cooccurrence_buffer(cooccurrence_counter* counter, std::size_t max_bytes,
      ...
      : counter_{counter},
        ...
        cooccurrences_{static_cast<std::size_t>(max_bytes_ / sizeof(count_t))},
  ...
  void flush() {
...

    {
      ...
      std::sort(
          ...
          [](const count_t& a, const count_t& b) { return a.first < b.first; });
 ...
 private:
  void maybe_flush() {
    ...
    if (cooccurrences_.next_load_factor() >= cooccurrences_.max_load_factor()) {
...
namespace {
hashing::probe_map<std::string, uint64_t> load_vocab(
...
void cooccurrence_counter::count(corpus::corpus& docs,
                                 const analyzers::token_stream& stream) {
  if (chunk_num_ != 0)
    throw cooccurrence_exception{"cooccurrence_counters may not be re-used"};
      ...
      [&](cooccurrence_buffer& buffer, const corpus::document& doc) {
        ...
        while (*buffer.stream_) {
          ...
          if (tok == "<s>" && break_on_tags_) {
            ...
            // history.
            for (auto it = history.begin(), end = history.end(); it != end;
                 ++it) {
            ...
            if (history.size() > window_size_) history.pop_front();
          }
        }
      });
}
...
void cooccurrence_counter::flush_chunk(memory_chunk_type&& chunk) {
  ...
  if (!chunk.empty()) memory_chunks_.emplace_back(std::move(chunk));
...
void cooccurrence_counter::memory_merge_chunks() {
  if (!memory_chunks_.empty()) {
    ...
    uint64_t total_bytes = 0;
    {
      ...
      printing::progress::clear();
      LOG(info) << "Merging " << memory_chunks_.size() << " in-memory chunks..."
                << ENDLG;
      util::multiway_merge(
          memory_chunks_.begin(), memory_chunks_.end(),
          [&](cooccur_record&& record) {
            total_bytes += io::packed::write(output, record);
          },
...
void cooccurrence_counter::maybe_merge() {
  std::unique_lock<std::mutex> lock{chunk_mutex_};
  if (chunks_.size() < merge_fanout_) return;
  ...
  uint64_t total_bytes = 0;
  {
    std::ofstream output{filename, std::ios::binary};
    printing::progress::clear();
    LOG(info) << "Merging " << chunks.size() << " on-disk chunks..." << ENDLG;
    lock.unlock();
