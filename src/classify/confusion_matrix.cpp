...
double confusion_matrix::precision(const class_label& lbl) const {
  ...
  for (auto& cls : classes_)
    denom +=
        map::safe_at(predictions_, std::make_pair(predicted_label{lbl}, cls));

  double correct =
      map::safe_at(predictions_, std::make_pair(predicted_label{lbl}, lbl));

  if (denom != 0.0) return correct / denom;
...
double confusion_matrix::recall(const class_label& lbl) const {
  ...
  for (auto& cls : classes_)
    denom +=
        map::safe_at(predictions_, std::make_pair(predicted_label{cls}, lbl));
...
void confusion_matrix::print(std::ostream& out) const {
  ...
  auto w = std::max<size_t>(7, static_cast<std::string>(*max_label).size()) + 2;
  ...
  for (auto& actual_class : classes_) {
    ...
    for (auto& pred_class : classes_) {
      ...
      if (it != predictions_.end()) {
        double percent =
            static_cast<double>(it->second) / counts_.at(actual_class);
        out.precision(3);
        std::stringstream ss;
        ss.precision(3);
        ss << percent;
...
void confusion_matrix::print_stats(std::ostream& out) const {
  ...
  auto w1 = std::setw(static_cast<int>(width + printing::make_bold("").size()));
  ...
  for (auto& cls : classes_) {
    ...
    out << std::left << std::setw(static_cast<int>(width)) << cls << std::left
        << w3 << f1_score(cls) << std::left << w3 << precision(cls) << std::left
        << w3 << recall(cls) << std::left << w3
        << static_cast<double>(counts_.at(cls)) / total_ << std::endl;
  }
...
double confusion_matrix::accuracy() const {
  ...
  for (auto& cls : classes_)
    correct +=
        map::safe_at(predictions_, std::make_pair(predicted_label{cls}, cls));
...
const confusion_matrix::prediction_counts& confusion_matrix::predictions()
    const {
  return predictions_;
}

confusion_matrix confusion_matrix::operator+(
    const confusion_matrix& other) const {
...
bool confusion_matrix::mcnemar_significant(const confusion_matrix& a,
  ...
  for (auto& cls : classes) {
    auto a_count =
        map::safe_at(a.predictions_, std::make_pair(predicted_label{cls}, cls));
    auto b_count =
        map::safe_at(b.predictions_, std::make_pair(predicted_label{cls}, cls));
    if (a_count > b_count)
  ...
  if (a_adv + b_adv < 25) return false;
