...
naive_bayes::naive_bayes(dataset_view_type docs, double alpha, double beta)
  ...
  for (const auto& lbl : labels) term_probs_.emplace_back(lbl, term_prior);
...
template <>
std::unique_ptr<classifier> make_classifier<naive_bayes>(
    const cpptoml::table& config, multiclass_dataset_view training) {
  auto alpha =
      config.get_as<double>("alpha").value_or(naive_bayes::default_alpha);

  auto beta = config.get_as<double>("beta").value_or(naive_bayes::default_beta);
