...
logistic_regression::logistic_regression(multiclass_dataset_view docs,
  ...
  parallel::parallel_for(
      classifiers_.begin(), classifiers_.end(), [&](T& pair) {
        ...
        binary_dataset_view bdv{docs, std::move(train_docs),
...
std::unordered_map<class_label, double> logistic_regression::predict(
    const feature_vector& doc) const {
  ...
  probs[pivot_] = 1;
  denom += 1;
  for (auto& pair : probs) pair.second /= denom;
  return probs;
}
...
class_label logistic_regression::classify(const feature_vector& doc) const {
  ...
  auto it = argmax(
      ...
      [](const std::pair<class_label, double>& pair) { return pair.second; });
  return it->first;
}
