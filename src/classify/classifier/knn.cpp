...
knn::knn(multiclass_dataset_view docs,
  ...
  for (const auto& instance : docs) legal_docs_.insert(doc_id(instance.id));
}

knn::knn(std::istream& in) : weighted_{io::packed::read<bool>(in)} {
...
void knn::save(std::ostream& out) const {
  ...
  for (const auto& doc : legal_docs_) io::packed::write(out, doc);
}
...
class_label knn::classify(const feature_vector& instance) const {
  ...
  auto scored = ranker_->score(
      *inv_idx_, query.begin(), query.end(), k_,
      [&](doc_id d_id) { return legal_docs_.find(d_id) != legal_docs_.end(); });
  ...
  for (auto& s : scored) {
    ...
    if (weighted_) counts[inv_idx_->label(s.d_id)] += s.score;
    // if not weighted, each neighbor gets an equal vote
    else
      ++counts[inv_idx_->label(s.d_id)];
  }

  if (counts.empty()) throw knn_exception{"label counts were empty"};
...
class_label knn::select_best_label(
  ...
  if (best.size() == 1) return *best.begin();
