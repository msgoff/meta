...
one_vs_all::one_vs_all(multiclass_dataset_view docs,
      ...
      [&](std::pair<const class_label, std::unique_ptr<binary_classifier>>&
        ...
        if (auto cls =
          ...
          throw classifier_exception{
              "base type in one_vs_all is not an "
              "online_binary_classifier"};
        }
      });
}

void one_vs_all::train_one(const feature_vector& doc,
...
template <>
std::unique_ptr<classifier> make_classifier<one_vs_all>(
