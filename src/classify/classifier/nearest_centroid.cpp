...
class_label nearest_centroid::classify(const feature_vector& instance) const {
  ...
  for (auto& centroid : centroids_) {
    double score = cosine_sim(counts.begin(), counts.end(), centroid.second);
    if (score > best_score) {
      best_score = score;
...
double nearest_centroid::cosine_sim(
  ...
  for (const auto& pair : centroid) centroid_mag += pair.second * pair.second;
  ...
  for (; begin != end; ++begin) {
    ...
    if (it != centroid.end()) dot += it->second * begin->second;
  }
...
std::unique_ptr<classifier> make_multi_index_classifier<nearest_centroid>(
  ...
  return make_unique<nearest_centroid>(std::move(training), std::move(inv_idx));
}
}  // namespace classify
}  // namespace meta
