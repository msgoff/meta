...
void dual_perceptron::train(multiclass_dataset_view docs) {
  for (auto it = docs.labels_begin(), end = docs.labels_end(); it != end; ++it)
    weights_[it->first] = {};
  ...
  for (uint64_t iter = 0; iter < max_iter_; ++iter) {
    ...
    if (static_cast<double>(error_count) / docs.size() < gamma_) break;
  }
}
...
void dual_perceptron::decrease_weight(const class_label& label,
  ...
  if (it == weights_[label].end()) return;
  --it->second;
  if (it->second == 0) weights_[label].erase(it);
}
...
class_label dual_perceptron::classify(const feature_vector& doc) const {
  ...
  for (const auto& w : weights_) {
    ...
    for (const auto& mistakes : w.second) {
      dot +=
          mistakes.second * ((*kernel_)(doc, svs_.at(mistakes.first)) + bias_);
    }
    dot *= alpha_;
    if (dot > best_dot) {
      best_dot = dot;
...
std::unique_ptr<classifier> make_classifier<dual_perceptron>(
  ...
  auto alpha =
      config.get_as<double>("alpha").value_or(dual_perceptron::default_alpha);

  auto gamma =
      config.get_as<double>("gamma").value_or(dual_perceptron::default_gamma);
