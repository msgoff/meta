...
one_vs_one::one_vs_one(multiclass_dataset_view docs,
                       const cpptoml::table& base) {
...
#ifndef NDEBUG
  for (const auto& pr : classifiers_) assert(pr.second != nullptr);
#endif
}
...
one_vs_one::one_vs_one(std::istream& in) {
  ...
  for (std::size_t i = 0; i < size; ++i) {
    ...
    classifiers_.emplace(problem_type{pos, neg}, load_binary_classifier(in));
  }
}
      ...
      [&](classifier_map_type::value_type& problem) {
        ...
        if (auto cls =
          ...
          throw classifier_exception{
              "base type in one_vs_one is not an "
              "online_binary_classifier"};
        }
      });
}

void one_vs_one::train_one(const feature_vector& doc,
...
template <>
std::unique_ptr<classifier> make_classifier<one_vs_one>(
