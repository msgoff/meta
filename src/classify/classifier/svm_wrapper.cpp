...
#ifndef _WIN32
  std::string command =
      svm_path_ + executable_ + "train " + options_.at(kernel_) + " svm-train";
...
svm_wrapper::svm_wrapper(std::istream& in)
  ...
  labels_.resize(size);
  for (std::size_t i = 0; i < size; ++i) io::packed::read(in, labels_[i]);
...
void svm_wrapper::save(std::ostream& out) const {
  ...
  for (const auto& lbl : labels_) io::packed::write(out, lbl);
