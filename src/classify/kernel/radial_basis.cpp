...
radial_basis::radial_basis(std::istream& in) { io::packed::read(in, gamma_); }
...
template <>
std::unique_ptr<kernel> make_kernel<radial_basis>(
    const cpptoml::table& config) {
  auto gamma = config.get_as<double>("gamma");
  if (!gamma)
    throw kernel_factory::exception{"rbf kernel needs gamma in configuration"};
