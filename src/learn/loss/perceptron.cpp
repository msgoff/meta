...
double perceptron::loss(double prediction, double expected) const {
  if (prediction * expected <= 0) return -expected * prediction;
  return 0;
}

double perceptron::derivative(double prediction, double expected) const {
  if (prediction * expected <= 0) return -expected;
  return 0;
}

void perceptron::save(std::ostream& out) const { io::packed::write(out, id); }
}  // namespace loss
}  // namespace learn
}  // namespace meta
