...
double sgd_model::train_one(const feature_vector& x, double expected_label,
  ...
  for (const auto& pr : x) {
    ...
    if (weight_val.scale > 0)
      update_scale_ +=
          (pr.second * pr.second) / (weight_val.scale * weight_val.scale);
  ...
  if (scale_ < 1e-10) {
    for (auto& weight_val : weights_) weight_val.weight *= scale_;
    bias_.weight *= scale_;
    scale_ = 1;
  }

  auto delta = -lr_ * std::sqrt(t_ / update_scale_) * error_derivative / scale_;
  if (delta != 0.0) {
    for (const auto& pr : x) {
      if (pr.second == 0.0) continue;
      ...
      weight_val.weight +=
          delta * 1.0 /
          (weight_val.scale * std::sqrt(weight_val.grad_squared)) * pr.second;

      // handle the L1 penalization
      if (l1_regularization_ > 0) penalize(weight_val);
    }
...
double sgd_model::l2norm() const {
  auto norm = 0.0;
  for (const auto& w : weights_) norm += scale_ * w.weight * w.weight;
  return norm;
}
