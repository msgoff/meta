...
libsvm_corpus::libsvm_corpus(const std::string& file,
  ...
  // to count newlines
  if (num_lines_ == 0) num_lines_ = filesystem::num_lines(file);

  // read first document
  std::getline(input_, next_content_);
}

bool libsvm_corpus::has_next() const { return !next_content_.empty(); }
...
uint64_t libsvm_corpus::size() const { return num_lines_; }

template <>
std::unique_ptr<corpus> make_corpus<libsvm_corpus>(
    util::string_view prefix, util::string_view dataset,
