...
std::unique_ptr<corpus> make_corpus(const cpptoml::table& config) {
  ...
  if (!corp) throw corpus_exception{"corpus missing from configuration file"};

  auto prefix = config.get_as<std::string>("prefix");
  if (!prefix) throw corpus_exception{"prefix missing from configuration file"};
  ...
  auto result =
      corpus_factory::get().create(*type, *prefix, *dataset, *corpus_config);
