...
metadata::schema_type corpus::schema() const {
  ...
  if (store_full_text())
    schema.insert(schema.begin(), metadata::field_info{
                                      "content", metadata::field_type::STRING});
  return schema;
}

const std::string& corpus::encoding() const { return encoding_; }
...
bool corpus::store_full_text() const { return store_full_text_; }
}  // namespace corpus
}  // namespace meta
