...
line_corpus::line_corpus(const std::string& file, std::string encoding,
  ...
  if (filesystem::file_exists(file + ".labels")) {
    ...
    if (num_lines_ == 0) num_lines_ = filesystem::num_lines(file + ".labels");
  }
...
bool line_corpus::has_next() const { return cur_id_ < size(); }

document line_corpus::next() {
  class_label label{"[none]"};

  if (class_infile_) *class_infile_ >> label;
...
uint64_t line_corpus::size() const { return num_lines_; }
