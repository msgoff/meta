...
auto sr_parser::state_analyzer::featurize(const state& state) const
    -> feature_vector {
  ...
  if (state.queue_size() == 0) {
    feats["queue_empty"] = 1;
    if (state.stack_size() == 1) feats["queue_empty_stack_single"] = 1;
  }
...
void sr_parser::state_analyzer::unigram_stack_feats(
    const node* n, std::string prefix, feature_vector& feats) const {
  node_info hi{n};
...
void sr_parser::state_analyzer::bigram_featurize(const state& state,
                                                 feature_vector& feats) const {
  bigram_features(state.stack_item(0), "s0", state.queue_item(0), "q0", feats);
  bigram_features(state.stack_item(0), "s0", state.stack_item(1), "s1", feats);
  bigram_features(state.stack_item(1), "s1", state.queue_item(0), "q0", feats);
  bigram_features(state.queue_item(0), "q0", state.queue_item(1), "q1", feats);
}
...
void sr_parser::state_analyzer::children_featurize(
    const state& state, feature_vector& feats) const {
...
const node* left_dependent(const node* n) {
  if (n) {
    ...
    while (!n->is_leaf()) {
      ...
      if (chi.head_word != hi.head_word) return child;
...
void sr_parser::state_analyzer::dependents_featurize(
    const state& state, feature_vector& feats) const {
