...
void pretty_print(std::ostream& os, const node* n, uint64_t depth) {
  ...
  if (n->is_leaf()) {
    ...
    if (auto word = leaf.word()) os << " " << *leaf.word();
    os << ")\n";
  } else {
    const auto& inode = n->as<internal_node>();
    os << padding << "(" << inode.category() << "\n";
    inode.each_child(
        [&](const node* child) { pretty_print(os, child, depth + 2); });
    os << padding << ")\n";
  }
}
...
void print(std::ostream& os, const node* n) {
  if (n->is_leaf()) {
    const auto& leaf = n->as<leaf_node>();
    os << "(" << leaf.category();
    if (auto word = leaf.word()) os << " " << *word;
    os << ")";
  } else {
    const auto& inode = n->as<internal_node>();
    os << "(" << inode.category();
...
void parse_tree::swap(parse_tree& other) { std::swap(root_, other.root_); }
