...
struct lexicon_populator : public visitor<void> {
  void operator()(leaf_node&) override { return; }

  void operator()(internal_node& inode) override {
    inode.each_child([&](node* child) { child->accept(*this); });

    if (!inode.head_lexicon()) inode.head(inode.head_constituent());
  }
};
}  // namespace
...
std::unique_ptr<node> binarizer::operator()(const internal_node& in) {
  ...
  if (!head) throw tree_binarizer_exception{"Head constituent not labeled"};

  uint64_t head_idx = 0;
  for (uint64_t idx = 0; idx < in.num_children(); ++idx) {
    if (in.child(idx) == in.head_constituent()) head_idx = idx;
  }
