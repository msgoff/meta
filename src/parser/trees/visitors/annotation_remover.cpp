...
std::string normalize(const std::string& cat) {
  ...
  if (dash != cat.npos && dash < eq && dash < pipe) return cat.substr(0, dash);
  if (eq != cat.npos && eq < dash && eq < pipe) return cat.substr(0, eq);
  if (pipe != cat.npos && pipe < dash && pipe < eq) return cat.substr(0, pipe);
  return cat;
}
}  // namespace
...
std::unique_ptr<node> annotation_remover::operator()(
    const internal_node& inode) {
