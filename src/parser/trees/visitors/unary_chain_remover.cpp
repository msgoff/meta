...
std::unique_ptr<node> unary_chain_remover::operator()(
  ...
  if (inode.num_children() > 1) {
    ...
    inode.each_child([&](const node* n) { ret->add_child(n->accept(*this)); });
    return std::move(ret);
  }
  ...
  if (n->is_leaf()) {
    ...
    if (inode.category() != in.category()) {
      ...
      in.each_child(
          [&](const node* child) { ret->add_child(child->accept(*this)); });
    }
    return std::move(ret);
  }
}
