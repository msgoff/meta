  ...
  uint64_t find_head(const internal_node& inode) const override {
    for (const auto& cand : candidates_) {
      for (uint64_t idx = 0; idx < inode.num_children(); ++idx) {
        ...
        if (child->category() == cand) return idx;
      }
    }
  ...
  uint64_t find_head(const internal_node& inode) const override {
    for (const auto& cand : candidates_) {
      for (uint64_t idx = 0; idx < inode.num_children(); ++idx) {
        ...
        if (child->category() == cand) return ridx;
      }
    }
...
struct head_np : public head_rule {
  ...
  uint64_t find_head(const internal_node& inode) const override {
    head_final_np first_pass{
        {"NN"_cl, "NNP"_cl, "NNPS"_cl, "NNS"_cl, "NX"_cl, "POS"_cl, "JJR"_cl}};
    if (auto idx = first_pass.find_head(inode)) return *idx;

    head_initial_np second_pass{{"NP"_cl}};
    if (auto idx = second_pass.find_head(inode)) return *idx;

    head_final_np third_pass{{"$"_cl, "ADJP"_cl, "PRN"_cl}};
    if (auto idx = third_pass.find_head(inode)) return *idx;

    head_final_np fourth_pass{{"CD"_cl}};
    if (auto idx = fourth_pass.find_head(inode)) return *idx;

    head_final_np fifth_pass{{"JJ"_cl, "JJS"_cl, "RB"_cl, "QP"_cl}};
    if (auto idx = fifth_pass.find_head(inode)) return *idx;
...
head_finder::head_finder() {
  ...
  rules_["NAC"_cl] = make_unique<head_initial>(
      ...
      "$"_cl, "CD"_cl, "QP"_cl, "PRP"_cl, "VBG"_cl, "JJ"_cl, "JJS"_cl, "JJR"_cl,
      "ADJP"_cl, "FW"_cl);
  ...
  rules_["RRC"_cl] =
      make_unique<head_final>("VP"_cl, "NP"_cl, "ADVP"_cl, "ADJP"_cl, "PP"_cl);

  rules_["S"_cl] =
      make_unique<head_initial>("TO"_cl, "IN"_cl, "VP"_cl, "S"_cl, "SBAR"_cl,
                                "ADJP"_cl, "UCP"_cl, "NP"_cl);

  rules_["SBAR"_cl] = make_unique<head_initial>(
      "WHNP"_cl, "WHPP"_cl, "WHADVP"_cl, "WHADJP"_cl, "IN"_cl, "DT"_cl, "S"_cl,
      "SQ"_cl, "SINV"_cl, "SBAR"_cl, "FRAG"_cl);

  rules_["SBARQ"_cl] = make_unique<head_initial>("SQ"_cl, "S"_cl, "SINV"_cl,
                                                 "SBARQ"_cl, "FRAG"_cl);

  rules_["SINV"_cl] =
      make_unique<head_initial>("VBZ"_cl, "VBD"_cl, "VBP"_cl, "VB"_cl, "MD"_cl,
                                "VP"_cl, "S"_cl, "SINV"_cl, "ADJP"_cl, "NP"_cl);
...
void head_finder::operator()(internal_node& inode) {
  ...
  inode.each_child([&](node* child) { child->accept(*this); });
