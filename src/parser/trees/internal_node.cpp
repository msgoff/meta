...
internal_node::internal_node(const internal_node& other)
  ...
  other.each_child([&](const node* n) {
    ...
    if (n == other.head_constituent()) head(children_.back().get());
  });
}
...
uint64_t internal_node::num_children() const { return children_.size(); }

bool internal_node::is_leaf() const { return false; }

bool internal_node::equal(const node& other) const {
  if (other.is_leaf() || category() != other.category()) return false;

  const auto& internal = other.as<internal_node>();

  if (num_children() != internal.num_children()) return false;
...
const leaf_node* internal_node::head_lexicon() const { return head_lexicon_; }

void internal_node::head_lexicon(const leaf_node* l) { head_lexicon_ = l; }

const node* internal_node::head_constituent() const {
  return head_constituent_;
}

void internal_node::head_constituent(const node* n) { head_constituent_ = n; }

void internal_node::head(const node* n) {
#if DEBUG
  bool found = false;
  for (const auto& child : children_) found = found || (child.get() == n);
  if (!found) throw std::runtime_error{"Cannot set head to a non-child node"};
#endif
  head_constituent(n);
