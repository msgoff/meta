...
class constituent_finder : public const_visitor<void> {
 public:
  void operator()(const leaf_node&) override { ++curr_leaf_; }
  ...
  void operator()(const internal_node& in) override {
    ...
    in.each_child([&](const node* child) { child->accept(*this); });
    con.end = curr_leaf_;

    constituents_.emplace(std::move(con));
  }

  std::multiset<constituent> constituents() { return std::move(constituents_); }
...
class collinizer : public tree_transformer {
 public:
  std::unique_ptr<node> operator()(const leaf_node& ln) override {
    ...
    if (punct_cats.find(ln.category()) != punct_cats.end()) return nullptr;
  ...
  std::unique_ptr<node> operator()(const internal_node& in) override {
    ...
    if (in.category() == "ROOT"_cl) return in.child(0)->accept(*this);
    ...
    in.each_child([&](const node* c) {
      ...
      if (child) res->add_child(std::move(child));
    });
...
void evalb::add_tree(parse_tree proposed, parse_tree gold) {
  ...
  if (matched == gold_total && matched == prop_const.size()) ++perfect_;
