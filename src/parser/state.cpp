...
state::state(const sequence::sequence& sentence)
  ...
  for (const auto& obs : sentence) {
    if (!obs.tagged()) throw sr_parser_exception{"sentence must be POS tagged"};
...
bool shift_legal(const state& state) {
  ...
  if (state.queue_size() == 0) return false;
...
bool unary_legal(const state& state, const transition& trans) {
  ...
  if (state.stack_size() == 0) return false;

  // Only FINALIZE is legal after ROOT
  if (state.stack_item(0)->category() == "ROOT"_cl) return false;
  ...
  // no unary chains
  if (state.stack_item(0)->category() == trans.label()) return false;
...
bool reduce_legal(const state& state, const transition& trans) {
  ...
  if (state.stack_size() < 2) return false;
  ...
  if (state.stack_size() > 2 && state.stack_item(2)->is_temporary()) {
    ...
    if (state.queue_size() == 0 && is_temporary(trans.label())) return false;
...
bool idle_legal(const state& state) { return state.finalized(); }
}  // namespace
...
size_t state::stack_size() const { return stack_.size(); }

size_t state::queue_size() const { return queue_->size() - q_idx_; }

const node* state::stack_item(size_t depth) const {
  if (depth >= stack_size()) return nullptr;

  auto st = stack_;
  for (uint64_t i = 0; i < depth; ++i) st = st.pop();
  return st.peek().get();
}

const leaf_node* state::queue_item(int64_t depth) const {
  auto idx = static_cast<std::size_t>(static_cast<int64_t>(q_idx_) + depth);
  if (idx < queue_->size()) return queue_->at(idx).get();
  return nullptr;
}

bool state::finalized() const { return done_; }
}  // namespace parser
}  // namespace meta
