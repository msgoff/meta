...
parse_tree sr_parser::parse(const sequence::sequence& sentence) const {
  if (sentence.size() == 0) return {make_unique<internal_node>("ROOT"_cl)};
  ...
  if (beam_size_ == 1) {
    while (!st.finalized()) {
      ...
      if (!st.legal(trans)) trans = st.emergency_transition();
    ...
    struct comparator {
      bool operator()(const scored_state& lhs, const scored_state& rhs) const {
    ...
    while (!std::all_of(agenda.begin(), agenda.end(), fin)) {
      ...
      for (const auto& ss : agenda) {
        ...
        auto transitions = best_transitions(feats, c_state, beam_size_, true);
      ...
      if (new_agenda.size() == 0) throw sr_parser_exception{"unparsable"};

      agenda = std::move(new_agenda);
    }

    // min because comp is backwards
    auto best = std::min_element(agenda.begin(), agenda.end(), comparator{});
...
void sr_parser::train(std::vector<parse_tree>& trees,
  ...
  for (uint64_t iter = 1; iter <= options.max_iterations; ++iter) {
    ...
    auto time = common::time([&]() {
      ...
      for (size_t start = 0; start < data.size(); start += options.batch_size) {
        progress(start);
        auto end = std::min<size_t>(start + options.batch_size, data.size());
...
auto sr_parser::train_batch(training_batch batch, parallel::thread_pool& pool,
  ...
  for (const auto& tid : pool.thread_ids()) updates[tid] = {};
  ...
  for (const auto& thread_update : updates) {
    for (const auto& feat : thread_update.second) {
      ...
      for (const auto& weight : feat.second) wv[weight.first] += weight.second;
    }
  }
...
std::pair<uint64_t, uint64_t> sr_parser::train_early_termination(
    const parse_tree& tree, const std::vector<trans_id>& transitions,
...
std::pair<uint64_t, uint64_t> sr_parser::train_beam_search(
  ...
  for (const auto& gold_trans : transitions) {
    ...
    for (const auto& ss : agenda) {
      ...
      auto transitions = best_transitions(feats, st, options.beam_size, true);
      ...
      for (const auto& scored_trans : transitions) {
        ...
        auto new_ss = std::make_tuple(new_state, new_score, new_is_gold);
