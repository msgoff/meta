...
int main(int argc, char** argv) {
  ...
  std::vector<parser::parse_tree> training;
  {
    ...
    for (auto i = static_cast<uint8_t>(begin); i <= end; ++i) {
      ...
      for (uint8_t j = 0; j <= *section_size; ++j) {
        ...
        for (auto& tree : trees) training.emplace_back(std::move(tree));
      }
    }
  }
  ...
  if (num_threads) options.num_threads = static_cast<uint64_t>(*num_threads);
  ...
  if (algorithm) {
    if (*algorithm == "early-termination") {
      ...
      options.algorithm = parser::sr_parser::training_algorithm::BEAM_SEARCH;

      auto beam_size = parser_grp->get_as<int64_t>("beam-size");
      if (beam_size) options.beam_size = static_cast<uint64_t>(*beam_size);

      LOG(info) << "Training using beam search (of size " << options.beam_size
                << ")" << ENDLG;
    } else {
      LOG(fatal) << "Unrecognized training algorithm: " << *algorithm << ENDLG;
      return 1;
    }
  }
