...
struct annotation_checker : public parser::const_visitor<bool> {
  bool operator()(const parser::leaf_node&) override { return true; }
  ...
  bool operator()(const parser::internal_node& inode) override {
    ...
    if (!inode.head_lexicon()) {
      std::cout << "Node missing head lex: " << inode.category() << std::endl;
      return false;
    }

    bool res = true;
    inode.each_child(
        [&](const parser::node* child) { res = res && child->accept(*this); });
    return res;
  }
};
...
struct binary_checker : public parser::const_visitor<bool> {
  ...
  bool operator()(const parser::internal_node& inode) override {
    if (inode.num_children() > 2) return false;
  ...
  for (size_t arg = 1; arg < args.size(); ++arg) {
    ...
    for (size_t i = 0; i < orig_trees.size(); ++i) {
      ...
      if (!t.visit(bin_check))
        throw std::runtime_error{"Binarizer failed to fully binarize the tree"};
      if (!t.visit(ann_check))
        throw std::runtime_error{"Binarized tree missing head annotations"};
