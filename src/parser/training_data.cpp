...
auto sr_parser::training_data::preprocess() -> transition_map {
  ...
  for (auto& tree : trees_) {
    ...
    for (const auto& trans : transitions) tids.push_back(trans_map[trans]);
...
size_t sr_parser::training_data::size() const { return indices_.size(); }

const parse_tree& sr_parser::training_data::tree(size_t idx) const {
  return trees_[indices_[idx]];
}

auto sr_parser::training_data::transitions(size_t idx) const
