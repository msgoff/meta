...
void throw_if_error(lzma_ret code, std::string msg) {
  switch (code) {
    ...
    case LZMA_BUF_ERROR:
      throw xz_exception{msg + ": Compressed file is truncated or corrupt",
                         code};
    case LZMA_UNSUPPORTED_CHECK:
      throw xz_exception{msg + ": Specified integrity check is not supported",
                         code};
    default:
...
xzstreambuf::xzstreambuf(const char* filename, const char* openmode,
  ...
  if (mode == "wb") {
    ...
    throw_if_error(
        lzma_stream_decoder(&stream_, std::numeric_limits<uint64_t>::max(), 0),
...
auto xzstreambuf::underflow() -> int_type {
  ...
  do {
    if (stream_.avail_in == 0 && !std::feof(file_)) {
      ...
      stream_.avail_in =
          std::fread(&in_buffer_[0], sizeof(uint8_t), in_buffer_.size(), file_);
...
int xzstreambuf::sync() {
  ...
  do {
    ...
    if (stream_.avail_out == 0 || ret == LZMA_STREAM_END) {
      ...
      if (std::fwrite(&out_buffer_[0], sizeof(uint8_t), size, file_) != size)
        return -1;
  ...
  if (bytes > 0) pbump(-static_cast<int>(bytes));
...
uint64_t xzstreambuf::bytes_read() const { return bytes_read_; }
...
void xzifstream::flush() { buffer_.sync(); }

uint64_t xzifstream::bytes_read() const { return buffer_.bytes_read(); }
...
void xzofstream::flush() { buffer_.sync(); }
}  // namespace io
}  // namespace meta
