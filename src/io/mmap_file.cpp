...
mmap_file::mmap_file(const std::string& path)
  ...
  if (file_descriptor_ < 0)
    throw mmap_file_exception{"error obtaining file descriptor for " + path_};

  start_ =
      (char*)mmap(nullptr, size_, PROT_READ, MAP_SHARED, file_descriptor_, 0);
  if (start_ == nullptr) {
...
char mmap_file::operator[](uint64_t index) const {
  if (index > size_) throw mmap_file_exception{"index out of bounds"};

  return start_[index];
}

char* mmap_file::begin() const { return start_; }
...
uint64_t mmap_file::size() const { return size_; }

std::string mmap_file::path() const { return path_; }
...
bool mmap_ifstream::is_open() const { return static_cast<bool>(file_); }

int mmap_ifstream::peek() const {
  if (!is_open() || pos_ >= file_->size()) return EOF;
...
void mmap_ifstream::close() { file_ = util::nullopt; }
}  // namespace io
}  // namespace meta
