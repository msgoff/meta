...
auto gzstreambuf::underflow() -> int_type {
  ...
  auto bytes =
      gzread(file_, &buffer_[0], static_cast<unsigned>(buffer_.size()));
  if (bytes <= 0) {
...
bool gzstreambuf::is_open() const { return file_ != nullptr; }
...
void gzifstream::flush() { buffer_.sync(); }
...
void gzofstream::flush() { buffer_.sync(); }
}  // namespace io
}  // namespace meta
