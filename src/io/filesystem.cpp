...
bool exists(const std::string& filename) { return file_exists(filename); }

uint64_t file_size(const std::string& filename) {
  if (!file_exists(filename)) return 0;

  traits::stat_data_type st;
  if (!traits::stat(filename.c_str(), &st)) return 0;
...
std::uintmax_t remove_all(const path_type& path) {
  if (!traits::file_exists(path.c_str())) return 0;

  traits::stat_data_type st;
  if (!traits::stat(path.c_str(), &st)) return 0;

  if (traits::is_file(&st) || traits::is_link(&st)) {
    if (traits::unlink_file(path.c_str())) return 1;
...
bool file_exists(const std::string& filename) { return fs::exists(filename); }

bool exists(const std::string& filename) { return fs::exists(filename); }
