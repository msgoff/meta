...
void progress::print() {
  ...
  std::fill(it, end, '=');
  *end = '>';
  if (end < barend) std::fill(end + 1, barend, ' ');
  it = barend;
  *it++ = ']';
  *it++ = ' ';

  it += ::sprintf(&(*it), "%d%%", static_cast<int>(percent * 100));
  it += ::sprintf(&(*it), " ETA %02d:%02d:%02d", std::min(999, hrs), mins % 60,
                  secs % 60);
...
void progress::print_endline(bool endline) { endline_ = endline; }
