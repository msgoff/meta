...
bit_vector_view::bit_vector_view(util::array_view<const uint64_t> data,
  ...
  if (META_UNLIKELY(math::integer::div_ceil(num_bits_, 64) > data.size()))
    throw std::out_of_range{
        "not enough bits in array view to "
...
util::array_view<const uint64_t> bit_vector_view::data() const { return data_; }

uint64_t bit_vector_view::size() const { return num_bits_; }
}  // namespace succinct
}  // namespace meta
