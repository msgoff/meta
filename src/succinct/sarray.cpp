...
sarray_builder::~sarray_builder() {
  ...
  if (!low_bits_) io::write_binary(low_stream_, uint64_t{0});
...
uint8_t sarray::num_low_bits() const { return num_low_bits_; }
...
uint64_t sarray_rank::rank(uint64_t i) const {
  ...
  if (high_query >= high_bit_zeroes_.num_positions()) return size();
  ...
  while (high_pos > 0 && high_bvv[high_pos - 1] &&
         low_bvv.extract((rank - 1) * num_low_bits, num_low_bits) >= low_val) {
    --rank;
    --high_pos;
  }
...
uint64_t sarray_select::size() const { return high_bit_ones_.num_positions(); }
}  // namespace succinct
}  // namespace meta
