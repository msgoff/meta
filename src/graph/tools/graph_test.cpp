...
void degree_dist(graph::undirected_graph<>& g, const std::string& outfile) {
  ...
  for (auto& c : degrees) out << c.first << " " << c.second << "\n";
}
...
void ring_world(double q) {
  ...
  for (uint64_t i = 0; i < g.size(); ++i) {
    ...
    for (uint64_t j = 0; j < g.size(); ++j) {
      if (i == j) continue;
      double prob = std::exp(std::log(static_cast<double>(j - i)) * -1.0 * q);
      choices.emplace_back(node_id{j}, prob);
      total_prob += prob;
    }

    for (auto& p : choices) p.second /= total_prob;
    ...
    for (auto& p : choices) {
      sum += p.second;
      if (sum > rand) {
        if (!g.edge(p.first, node_id{i})) g.add_edge(p.first, node_id{i});
        break;
      }
    }
  }
...
void simulations() {
  ...
  undirected_graph<> g2;
  algorithms::preferential_attachment(g2, num_nodes, num_edges, [&](node_id) {
  ...
  undirected_graph<> g3;
  algorithms::preferential_attachment(g3, num_nodes, num_edges, [&](node_id) {
    auto x = gamma1(gen);
  ...
  undirected_graph<> g4;
  algorithms::preferential_attachment(
      g4, num_nodes, num_edges,
      [&](node_id id) { return (id % 2) ? 0.1 : 1000.0; });
  ...
  algorithms::preferential_attachment(g5, num_nodes, num_edges,
                                      [&](node_id id) { return id * 10; });
  degree_dist(g5, "pa-increasing.dat");

  undirected_graph<> g6;
  algorithms::preferential_attachment(
      g6, num_nodes, num_edges, [&](node_id id) { return 1.0 / (id + 1); });
...
void hybrid(const std::string& graph_file) {
  ...
  for (auto& n : g) ++counts[g.adjacent(n.id).size()];

  std::ofstream out{"degrees.txt"};
  for (auto& c : counts) out << c << std::endl;
}
