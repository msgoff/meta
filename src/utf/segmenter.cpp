...
class segmenter::impl {
 public:
  ...
  impl() {
    ...
    word_iter_.reset(icu::BreakIterator::createWordInstance(locale, status));
    if (!U_SUCCESS(status))
  ...
  impl(const std::string& language,
    ...
    icu::Locale locale(language.c_str(), country ? country->c_str() : nullptr);
    if (locale.isBogus()) throw std::runtime_error{"failed to create locale"};
   ...
   */
  void set_content(util::string_view str) { text_ = str; }
   ...
   */
  enum class segment_t { SENTENCES, WORDS };
  ...
  std::vector<segment> words() const {
    return segments(0, static_cast<int32_t>(text_.length()), segment_t::WORDS);
  }
...
segmenter::segmenter() { icu_handle::get(); }
...
void segmenter::set_content(util::string_view str) { impl_->set_content(str); }

auto segmenter::sentences() const -> std::vector<segment> {
  return impl_->sentences();
}

auto segmenter::words() const -> std::vector<segment> { return impl_->words(); }
