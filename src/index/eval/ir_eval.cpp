...
ir_eval::ir_eval(const cpptoml::table& config) {
  ...
  if (!path) throw ir_eval_exception{"query judgement file was not specified"};
...
void ir_eval::init_index(const std::string& path) {
  ...
  while (in.good()) {
    ...
    if (relevance > 0) qrels_[q_id][d_id] = relevance;
  }
}
...
double ir_eval::relevant_retrieved(const std::vector<search_result>& results,
  ...
  for (auto& result : results) {
    if (map::safe_at(ht->second, result.d_id) != 0) ++rel;
    if (i++ == num_docs) break;
  }
...
double ir_eval::f1(const std::vector<search_result>& results, query_id q_id,
  ...
  if (denominator < 0.00000001) return 0.0;
...
double ir_eval::ndcg(const std::vector<search_result>& results, query_id q_id,
  ...
  const auto& ht = qrels_.find(q_id);
  if (ht == qrels_.end() || results.empty()) return 0.0;
  ...
  for (const auto& s : ht->second) rels.push_back(s.second);
...
double ir_eval::avg_p(const std::vector<search_result>& results, query_id q_id,
  ...
  for (auto& result : results) {
    ...
    if (num_rel - 1 == total_relevant) break;
    if (i++ == num_docs) break;
  }
...
void ir_eval::print_stats(const std::vector<search_result>& results,
  ...
  out << w1 << printing::make_bold("  F1 Score:") << w2 << std::setprecision(p)
      << f1(results, q_id, num_docs);
  out << w1 << printing::make_bold("  Precision:") << w2 << std::setprecision(p)
      << precision(results, q_id, num_docs);
  out << w1 << printing::make_bold("  Recall:") << w2 << std::setprecision(p)
      << recall(results, q_id, num_docs);
  out << std::endl;
}

void ir_eval::reset_stats() { scores_.clear(); }
}  // namespace index
}  // namespace meta
