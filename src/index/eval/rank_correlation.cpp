...
struct rank_pair {
  ...
  bool tied() const { return tied_x() && tied_y(); }

  bool tied_x() const { return xi == xj; }

  bool tied_y() const { return yi == yj; }
};
}  // namespace
...
double rank_correlation::gamma() const { return (nc() - nd()) / (nc() + nd()); }
...
double rank_correlation::tau_b() const {
  return (nc() - nd()) /
         std::sqrt((nc() + nd() + num_ties_x_) * (nc() + nd() + num_ties_y_));
}
