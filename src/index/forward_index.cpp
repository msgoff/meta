...
forward_index::impl::impl(forward_index* idx, const cpptoml::table& config)
    : idx_{idx} {
  if (!is_libsvm_analyzer(config)) analyzer_ = analyzers::load(config);
}
...
bool forward_index::valid() const {
  ...
  for (auto& f : impl_->files) {
    ...
    if (!filesystem::file_exists(index_name() + "/" + std::string{f})) {
      LOG(info) << "Existing forward index detected as invalid (missing " << f
                << "); recreating" << ENDLG;
      return false;
    }
  }
...
void forward_index::load_index() {
  ...
  if (!fwd_impl_->is_libsvm_analyzer(*config)) impl_->load_term_id_mapping();
  ...
  if (fwd_impl_->is_libsvm_analyzer(config)) {
    ...
    if (!dynamic_cast<corpus::libsvm_corpus*>(&docs))
      throw forward_index_exception{
          "both analyzer and corpus type must "
          "be libsvm in order to use libsvm "
          "formatted data"};

    LOG(info) << "Creating index from libsvm data: " << index_name() << ENDLG;
    ...
    if (config.get_as<bool>("uninvert").value_or(false)) {
      ...
      // RAM budget is given in MB
      fwd_impl_->tokenize_docs(docs, mdata_writer, ram_budget * 1024 * 1024,
                               num_threads);
      impl_->load_term_id_mapping();
      impl_->save_label_id_mapping();
      fwd_impl_->total_unique_terms_ = impl_->total_unique_terms();
  ...
  corpus::parallel_consume(
      ...
      [&]() {
        ...
        return local_storage{
            idx_->index_name() + "/chunk-" + std::to_string(cid), analyzer_};
      },
      [&](local_storage& ls, const corpus::document& doc) {
        ...
        if (counts.empty()) {
          std::lock_guard<std::mutex> lock{io_mutex};
          LOG(progress) << '\n' << ENDLG;
          LOG(warning) << "Empty document (id = " << doc.id() << ") generated!"
                       << ENDLG;
        }
        ...
        pd_counts.reserve(counts.size());
        {
          ...
          if (!exceeded_budget && vocab.bytes_used() > ram_budget) {
            ...
            LOG(progress) << '\n' << ENDLG;
            LOG(warning) << "Exceeding RAM budget; indexing cannot "
...
void forward_index::impl::merge_chunks(
  ...
  for (const auto& pr : vocab) keys[pr.value()] = pr.key();
...
bool forward_index::impl::is_libsvm_analyzer(
    const cpptoml::table& config) const {
  auto analyzers = config.get_table_array("analyzers")->get();
  if (analyzers.size() != 1) return false;

  auto method = analyzers[0]->get_as<std::string>("method");
  if (!method) throw forward_index_exception{"failed to find analyzer method"};
...
util::optional<postings_stream<term_id, double>> forward_index::stream_for(
    doc_id d_id) const {
