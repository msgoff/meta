...
uint64_t disk_index::num_labels() const { return impl_->label_ids_.size(); }
...
uint64_t disk_index::num_docs() const { return impl_->metadata_->size(); }
...
void disk_index::disk_index_impl::load_labels() {
  labels_ = util::disk_vector<const label_id>{index_name_ + files[DOC_LABELS]};
}
...
std::vector<class_label> disk_index::disk_index_impl::class_labels() const {
  ...
  for (const auto& pair : label_ids_) labels.emplace_back(pair.first);
  return labels;
}

std::string disk_index::term_text(term_id t_id) const {
  if (t_id >= impl_->term_id_mapping_->size()) return "";
