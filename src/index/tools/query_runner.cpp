...
int main(int argc, char* argv[]) {
  ...
  if (!group) throw std::runtime_error{"\"ranker\" group needed in config"};
  ...
  if (!query_path)
    throw std::runtime_error{"config file needs a \"query-path\" parameter"};
  ...
  auto max_results = query_group->get_as<uint64_t>("max-results").value_or(10);
  ...
  try {
    if (!trec_format) eval = make_unique<index::ir_eval>(*query_group);
  } catch (index::ir_eval_exception& ex) {
    LOG(info) << "Could not find relevance judgements; skipping eval" << ENDLG;
  }
  ...
  auto elapsed_seconds = common::time([&]() {
    while (std::getline(queries, content)) {
      ...
      if (!trec_format) {
        ...
        std::cout << "Query " << q_id << ": \"" << content << "\"" << std::endl;
        std::cout << std::string(80, '-') << std::endl;
      }
      ...
      for (auto& result : ranking) {
        ...
        if (result_num++ == max_results) break;
      }
      if (!trec_format && eval)
        eval->print_stats(ranking, query_id{q_id}, std::cout, max_results);
      ++q_id;
    }
  });
  ...
  if (!trec_format && eval) {
    ...
    std::cout << printing::make_bold(" gMAP: ") << eval->gmap() << std::endl;
    std::cout << std::endl;
  }
  std::cerr << "Elapsed time: " << elapsed_seconds.count() << "ms" << std::endl;
}
