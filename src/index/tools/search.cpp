...
int main(int argc, char* argv[]) {
  ...
  auto elapsed = common::time([&]() {
    ...
    for (size_t i = 0; i < 20 && i < idx->num_docs(); ++i) {
      auto path =
          idx->metadata<std::string>(docs[i], "path").value_or("[none]");
      ...
      std::cout << "Ranking query " << (i + 1) << ": " << path << std::endl;
      ...
      for (auto& result : ranking) {
        std::cout << result_num << ". "
                  << idx->metadata<std::string>(result.d_id, "name")
                         .value_or("[none]")
                  << " " << result.score << std::endl;
        if (result_num++ == 10) break;
      }
