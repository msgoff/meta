...
struct char_input_stream {
  ...
  char get() {
    if (input_ == end_)
      throw corpus::metadata_exception{"seeking past end of metadata file"};
...
corpus::metadata metadata_file::get(doc_id d_id) const {
  if (d_id >= index_.size())
    throw corpus::metadata_exception{"invalid doc id in metadata retrieval"};

  uint64_t seek_pos = index_[d_id];
  return {md_db_.begin() + seek_pos, schema_};
}

uint64_t metadata_file::size() const { return index_.size(); }
}  // namespace index
}  // namespace meta
