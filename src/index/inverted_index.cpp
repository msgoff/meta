...
bool inverted_index::valid() const {
  for (auto& f : impl_->files) {
    if (!filesystem::file_exists(index_name() + "/" + std::string{f})) {
      LOG(info) << "Existing inverted index detected as invalid; recreating"
                << ENDLG;
      return false;
    }
  }
...
void inverted_index::create_index(const cpptoml::table& config,
  ...
  auto max_writers = config.get_as<unsigned>("indexer-max-writers").value_or(8);
  ...
  inv_impl_->compress(index_name() + impl_->files[POSTINGS], num_unique_terms);
...
uint64_t inverted_index::total_corpus_terms() {
  if (inv_impl_->total_corpus_terms_ == 0) {
    for (auto& id : docs()) inv_impl_->total_corpus_terms_ += doc_size(id);
  }
...
analyzers::feature_map<uint64_t> inverted_index::tokenize(
...
util::optional<postings_stream<doc_id>> inverted_index::stream_for(
    term_id t_id) const {
