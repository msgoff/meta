...
vocabulary_map_writer::vocabulary_map_writer(const std::string& path,
  ...
  if (!file_ || !inverse_file_)
    throw vocabulary_map_writer_exception{"failed to open vocabulary map file"};
}
...
vocabulary_map_writer::~vocabulary_map_writer() {
  ...
  if (remaining_block_space_ != block_size_) flush();
  file_.flush();
  ...
  while (remaining_nodes != 1) {
    ...
    while (remaining_nodes > 0) {
      ...
      // 32-bit systems)
      auto length = static_cast<int64_t>(sizeof(uint64_t) + term.length() + 1);
