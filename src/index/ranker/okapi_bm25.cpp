...
float okapi_bm25::score_one(const score_data& sd) {
  ...
  float IDF = fastapprox::fastlog(1.0f + (sd.num_docs - sd.doc_count + 0.5f) /
                                             (sd.doc_count + 0.5f));

  float TF =
      ((k1_ + 1.0f) * sd.doc_term_count) /
      ((k1_ * ((1.0f - b_) + b_ * doc_len / sd.avg_dl)) + sd.doc_term_count);

  float QTF =
      ((k3_ + 1.0f) * sd.query_term_weight) / (k3_ + sd.query_term_weight);
...
std::unique_ptr<ranker> make_ranker<okapi_bm25>(const cpptoml::table& config) {
  ...
  if (k1 < 0) throw ranker_exception{"bm25 k1 must be >= 0"};

  if (k3 < 0) throw ranker_exception{"bm25 k3 must be >= 0"};

  if (b < 0 || b > 1) throw ranker_exception{"bm25 b must be on [0,1]"};
