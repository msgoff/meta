...
std::vector<search_result> ranker::score(
    inverted_index& idx, const corpus::document& query,
...
std::vector<search_result> ranking_function::rank(
  ...
  while (ctx.cur_doc < ctx.idx.num_docs()) {
    ...
    for (auto& pc : ctx.postings) {
      ...
      if (pc.begin != pc.end) {
        ...
        if (pc.begin->first < next_doc) next_doc = pc.begin->first;
      }
    }
...
float ranking_function::initial_score(const score_data&) const { return 0.0; }
}  // namespace index
}  // namespace meta
