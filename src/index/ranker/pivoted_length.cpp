...
pivoted_length::pivoted_length(float s) : s_{s} { /* nothing */ }
...
float pivoted_length::score_one(const score_data& sd) {
  ...
  float TF =
      1.0f + fastapprox::fastlog(1.0f + fastapprox::fastlog(sd.doc_term_count));
  float norm = (1.0f - s_) + s_ * (doc_len / sd.avg_dl);
  float IDF = fastapprox::fastlog((sd.num_docs + 1.0f) / (0.5f + sd.doc_count));
  return TF / norm * sd.query_term_weight * IDF;
}

template <>
std::unique_ptr<ranker> make_ranker<pivoted_length>(
