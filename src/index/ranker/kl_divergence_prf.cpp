...
std::vector<search_result> kl_divergence_prf::rank(
    ranker_context& ctx, uint64_t num_results,
...
template <>
std::unique_ptr<ranker> make_ranker<kl_divergence_prf>(
  ...
  if (global.begin() == global.end()) {
    ...
    LOG(fatal) << "Did you mean to call index::make_ranker(global, local) "
               ...
               << ENDLG;
    throw ranker_exception{
        "empty global configuration provided to "
        "construction of kl_divergence_prf ranker"};
  }

  auto alpha =
      local.get_as<double>("alpha").value_or(kl_divergence_prf::default_alpha);
