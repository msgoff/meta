...
language_model::language_model(const cpptoml::table& config) {
  ...
  if (binary_file && filesystem::file_exists(*binary_file + "0.binlm")) {
    LOG(info) << "Loading language model from binary files: " << *binary_file
              << "*" << ENDLG;
    auto time = common::time([&]() {
      prefix_ = *binary_file;
      load_vocab();
      while (
          filesystem::file_exists(*binary_file + std::to_string(N_) + ".binlm"))
        lm_.emplace_back(*binary_file + std::to_string(N_++) + ".binlm");
    });
...
void language_model::read_arpa_format(const std::string& arpa_file) {
  ...
  read_arpa(
      infile,
      [&](uint64_t /* order */, uint64_t ngramcount) {
        count.push_back(ngramcount);
      },
      [&](uint64_t order, const std::string& ngram, float prob, float backoff) {
        if (lm_.size() < order + 1) {
...
std::vector<std::pair<std::string, float>> language_model::top_k(
    const sentence& prev, size_t k) const {
  ...
  auto candidates = util::make_fixed_heap<pair_t>(
      k, [](const pair_t& a, const pair_t& b) { return a.second > b.second; });
...
term_id language_model::unk() const { return unk_id_; }
...
float language_model::score(const lm_state& in_state, term_id token,
  ...
  if (out_state.previous.size() > in_state.previous.size()) return res;
