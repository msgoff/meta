 ...
 */
lm::sentence tokenize_sentence(std::string& line,
                               const cpptoml::table& config) {
  auto group = config.get_table_array("analyzers");
  if (!group) throw std::runtime_error{"[[analyzers]] missing from config"};

  auto stream = analyzers::load_filters(config, *(group->get()[0]));
  if (!stream) throw std::runtime_error{"could not initialize token stream"};

  lm::sentence sent;  // create empty sentence
  stream->set_content(std::move(line));
  while (*stream) sent.push_back(stream->next());
...
int main(int argc, char* argv[]) {
  ...
  while (true) {
    ...
    double perplexity;
    auto time = common::time([&]() { perplexity = model.perplexity(sent); });
    std::cout << "Perplexity per word: " << perplexity << " (" << time.count()
              << "ms)" << std::endl;

    double log_prob;
    time = common::time([&]() { log_prob = model.log_prob(sent); });
