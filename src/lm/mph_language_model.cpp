 ...
 public:
      ...
      util::for_each_token(
          ...
          [&](std::string::const_iterator first,
            ...
            if (first != last) {
              ...
              if (!id)
                throw std::runtime_error{"ngram contains unknown unigram " +
                                         unigram.to_string()};
  ...
  void finish_order() {
    ...
    if (order_ == 0) {
      ...
      unigrams_ = make_unique<ngram_map<util::string_view>>(prefix_ + "/0");
...
uint64_t build_from_arpa(const std::string& arpa_file,
  ...
  read_arpa(
      arpa, [&](uint64_t /* order */, uint64_t count) { handler.count(count); },
      handler);
...
struct mph_language_model::impl {
  ...
  impl(const std::string& prefix, uint64_t ord)
    ...
    for (uint64_t i = 1; i < order; ++i) {
      auto mid = make_unique<middle_map_type>(prefix + "/" + std::to_string(i));
...
mph_language_model::mph_language_model(const cpptoml::table& config) {
  ...
  if (!filesystem::file_exists(*prefix + "/0/values.bin")) {
    ...
    auto time =
        common::time([&]() { order = build_from_arpa(*arpa_file, *prefix); });
...
term_id mph_language_model::unk() const { return term_id{impl_->unk.idx}; }
...
float mph_language_model::score(const lm_state& in_state, term_id token,
  ...
  if (out_state.previous.size() == 1) res = pb.prob;
