...
diff::diff(const cpptoml::table& config) : lm_{config} {
  ...
  if (!table) throw diff_exception{"missing [diff] table from config"};

  auto nval = table->get_as<uint64_t>("n-value");
  if (!nval) throw diff_exception{"n-value not specified in config"};
  n_val_ = *nval;

  auto edits = table->get_as<uint64_t>("max-edits");
  if (!edits) throw diff_exception{"max-edits not specified in config"};
  max_edits_ = *edits;
...
std::vector<std::pair<sentence, double>> diff::candidates(
    const sentence& sent, bool use_lm /* = false */) {
  use_lm_ = use_lm;
  ...
  if (lm_generate_ && best_idx >= n_val_) {
    ...
    try {
      for (auto& next : lm_.top_k(best, 3)) {
        if (next.first == "</s>") continue;

        sentence ins_cpy{sent};
        ins_cpy.insert(best_idx, next.first, base_penalty_ + insert_penalty_);
...
void diff::set_function_words(const cpptoml::table& config) {
  ...
  std::string word;
  while (in >> word) fwords_.push_back(word);
}
...
void diff::set_stems(const cpptoml::table& config) {
  ...
  std::string token;
  while (in >> token) vocab.insert(utf::foldcase(token));
