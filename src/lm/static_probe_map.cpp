...
void static_probe_map::insert(const token_list& key, float prob,
                              float backoff) {
  ...
  while (true) {
    ...
    if (table_[idx] == hashed)
      throw static_probe_map_exception{"key already exists (or collision)"};

    idx = (idx + 2) % table_.size();
  }
}

util::optional<lm_node> static_probe_map::find(
    const std::vector<term_id>& ngram) const {
...
util::optional<lm_node> static_probe_map::find_hash(uint64_t hashed) const {
  ...
  while (true) {
    if (table_[idx] == uint64_t{0}) return util::nullopt;

    if (table_[idx] == hashed) return {table_[idx + 1]};

    idx = (idx + 2) % table_.size();
  }
}

hashing::murmur_hash<>::result_type static_probe_map::hash(
