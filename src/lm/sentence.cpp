...
sentence::sentence(const std::string& text, bool tokenize /* = true */) {
  if (tokenize) {
    ...
    if (tokens_.empty()) throw sentence_exception{"empty token stream"};
  } else {
...
std::string sentence::to_string() const {
  ...
  for (auto& token : tokens_) result += token + " ";
...
sentence sentence::operator()(size_type from, size_type to) const {
  ...
  if (from > to || to > tokens_.size())
    throw sentence_exception{
        "operator() out of bounds: from = " + std::to_string(from) +
        ", to = " + std::to_string(to)};
...
const std::vector<std::string>& sentence::tokens() const { return tokens_; }

const std::string& sentence::front() const { return tokens_.front(); }

const std::string& sentence::back() const { return tokens_.back(); }
