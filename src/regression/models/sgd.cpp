...
sgd::sgd(std::istream& in)
    : model_{[&]() { return learn::sgd_model{in}; }()},
  ...
  for (size_t iter = 0; iter < max_iter_; ++iter) {
    ...
    for (const auto& instance : docs) {
      ...
      if (t % check_interval == 0) {
        ...
        if (prev_avg_loss > 0 &&
            std::abs(prev_avg_loss - avg_loss) / prev_avg_loss < gamma_)
          return;
      ...
      avg_loss +=
          model_.train_one(instance.weights, docs.label(instance), *loss_);
    }
  }
}
...
template <>
std::unique_ptr<regressor> make_regressor<sgd>(
    const cpptoml::table& config, regression_dataset_view training) {
