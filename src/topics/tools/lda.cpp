...
int run_lda(const std::string& config_file) {
  ...
  auto f_idx =
      index::make_index<index::forward_index, caching::no_evict_cache>(*config);
  if (type == "gibbs") {
    std::cout << "Beginning LDA using serial Gibbs sampling..." << std::endl;
    return run_lda<lda_gibbs>(f_idx, iters, topics, alpha, beta, save_prefix);
    ...
    std::cout << "Beginning LDA using stochastic collapsed variational bayes..."
              << std::endl;
    return run_lda<lda_scvb>(f_idx, iters, topics, alpha, beta, save_prefix);
  }
