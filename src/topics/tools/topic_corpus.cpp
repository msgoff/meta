...
void create_topic_corpus(const std::string& prefix, const std::string& dataset,
  ...
  auto new_file = prefix + "/" + dataset + "/" + dataset + "-topics.dat.labels";
  std::ofstream out_labels{new_file};
  size_t num_topics = 0;
  for (auto& topic : topic_ids) {
    if (topic > num_topics) num_topics = topic;
  ...
  std::string buf;
  while (orig_labels_in >> buf) orig_labels.push_back(buf);
  ...
  for (auto& topic : topic_ids) {
    ...
    if (counts[label].empty()) counts[label].resize(num_topics, 0.0);
    ++counts[label][topic];
    ++idx;
  }
  ...
  for (auto& label : counts) {
    ...
    double total = 0.0;
    for (const auto& count : label.second) total += count;
    for (const auto& count : label.second) out_dist << "\t" << count / total;
    out_dist << std::endl;
  }
...
int main(int argc, char* argv[]) {
  ...
  if (ctype != "line-corpus") {
    std::cerr << "Currently only line_corpus format is supported!" << std::endl;
    return 1;
  }
