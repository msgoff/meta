...
void lda_model::save_doc_topic_distributions(
...
void lda_model::save_topic_term_distributions(
    const std::string& filename) const {
  std::ofstream file{filename};
  ...
  for (topic_id j{0}; j < num_topics_; ++j) {
    ...
    for (term_id t_id{0}; t_id < idx_->unique_terms(); ++t_id) {
      ...
      if (norm_prob > 0) file << t_id << ":" << norm_prob << "\t";
    }
    file << "\n";
  }
}
...
uint64_t lda_model::num_topics() const { return num_topics_; }
}  // namespace topics
}  // namespace meta
