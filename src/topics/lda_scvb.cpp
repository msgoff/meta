...
void lda_scvb::initialize(std::mt19937& rng) {
  ...
  for (auto& v : topic_term_count_) v.resize(idx_->unique_terms());
  ...
  for (uint64_t j = 0; j < minibatch_size_; ++j) {
    ...
    for (const auto& freq : idx_->search_primary(d)->counts()) {
      ...
      for (topic_id k{0}; k < num_topics_; ++k) {
        ...
        doc_topic_count_[d][k] = weight * doc_topic_count_[d][k] +
    ...
    for (const auto& freq : idx_->search_primary(d)->counts()) {
      ...
      for (topic_id k{0}; k < num_topics_; ++k) {
        ...
        batch_topic_term_count_[k][freq.first] += idx_->num_docs() * gamma[k];
...
double lda_scvb::compute_doc_topic_probability(doc_id doc,
                                               topic_id topic) const {
