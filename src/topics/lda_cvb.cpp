...
lda_cvb::lda_cvb(std::shared_ptr<index::forward_index> idx,
  ...
  for (topic_id topic{0}; topic < num_topics_; ++topic)
    phi_.emplace_back(stats::dirichlet<term_id>{beta, idx_->unique_terms()});
}
...
void lda_cvb::run(uint64_t num_iters, double convergence) {
  ...
  for (uint64_t i = 0; i < num_iters; ++i) {
    ...
    ss << "Iteration " << i + 1 << " maximum change in gamma: " << max_change;
    std::string spacing(
    ...
    if (max_change <= convergence) {
      LOG(progress) << "Found convergence after " << i + 1 << " iterations!\n";
      break;
    }
  }
    ...
    for (auto& freq : idx_->search_primary(d)->counts()) {
      for (uint64_t count = 0; count < freq.second; ++count) {
        ...
        for (topic_id k{0}; k < num_topics_; ++k) {
          ...
          auto weight =
              phi_[k].probability(freq.first) * theta_[d].probability(k);
...
double lda_cvb::compute_doc_topic_probability(doc_id doc,
                                              topic_id topic) const {
