...
auto markov_model::expected_counts_type::operator+=(
    const expected_counts_type& other) -> expected_counts_type& {
...
markov_model::markov_model(expected_counts_type&& counts)
  ...
  auto inorm = std::accumulate(initial_prob_.begin(), initial_prob_.end(), 0.0);
  for (state_id s_i{0}; s_i < num_states(); ++s_i) {
    initial_prob_[s_i] = (initial_prob_[s_i] + prior_.pseudo_counts(s_i)) /
                         (inorm + prior_.pseudo_counts());

    auto tnorm =
        std::accumulate(trans_prob_.begin(s_i), trans_prob_.end(s_i), 0.0);
    for (state_id s_j{0}; s_j < num_states(); ++s_j) {
      trans_prob_(s_i, s_j) =
...
const stats::dirichlet<state_id>& markov_model::prior() const { return prior_; }

uint64_t markov_model::num_states() const { return initial_prob_.size(); }
