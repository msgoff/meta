...
void crf::load_model() {
  // ranges
  observation_ranges_ =
      util::disk_vector<crf_feature_id>{prefix_ + "/observation_ranges.vector"};
  transition_ranges_ =
      util::disk_vector<crf_feature_id>{prefix_ + "/transition_ranges.vector"};

  // observations
  observation_weights_ =
      util::disk_vector<double>{prefix_ + "/observation_weights.vector"};
  observations_ = util::disk_vector<label_id>{prefix_ + "/observations.vector"};
...
void crf::initialize(const std::vector<sequence>& examples) {
...

  {
    ...
    for (const auto& seq : examples) {
      ...
      for (uint64_t t = 0; t < seq.size(); ++t) {
        ...
        // transition features
        if (prev) trans_feats[*prev].insert(lbl);
  ...
  observations_ =
      util::disk_vector<label_id>{prefix_ + "/observations.vector", obs_size};
  ...
  transitions_ =
      util::disk_vector<label_id>{prefix_ + "/transitions.vector", trans_size};
...
void crf::reset() {
  for (auto& w : *observation_weights_) w = 0;
  for (auto& w : *transition_weights_) w = 0;
  scale_ = 1;
}

uint64_t crf::num_labels() const { return num_labels_; }
...
double crf::calibrate(parameters params, const std::vector<uint64_t>& indices,
  ...
  while (trial < params.calibration_trials) {
    ...
    if (std::isfinite(loss) && loss < initial_loss) {
      LOG(progress) << "\r" << ss.str() << "eta=" << eta << ", loss=" << loss
                    << " (possible)\n"
                    << ENDLG;
      ++trial;
  ...
  for (uint64_t t = 0; t < seq.size(); ++t) {
    for (const auto& pair : seq[t].features()) {
      for (const auto& idx : obs_range(pair.first)) {
        ...
        obs_weight(idx) += gain * pair.second * scr.state_marginal(t, lbl);
      }
    }
  }
...
double crf::l2norm() const {
  double norm = 0;
  for (const auto& w : *observation_weights_) norm += w * w;
  for (const auto& w : *transition_weights_) norm += w * w;
  return norm * scale_ * scale_;
}

void crf::rescale() {
  for (auto& w : *observation_weights_) w *= scale_;
  for (auto& w : *transition_weights_) w *= scale_;
  scale_ = 1;
}
}  // namespace sequence
}  // namespace meta
