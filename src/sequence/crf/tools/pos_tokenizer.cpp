...
int main(int argc, char* argv[]) {
  ...
  std::string word;
  while (keep_list_file >> word) keep_list.insert(word);
  ...
  if (!prefix) {
    std::cerr << "prefix to learned model needed in [crf] group" << std::endl;
    return 1;
  }
  ...
  while (std::getline(std::cin, line)) {
    ...
    std::unordered_set<std::string> ptb_special = {"-LRB-", "-RRB-", "-LSB-",
                                                   "-RSB-", "-LCB-", "-RCB-"};
