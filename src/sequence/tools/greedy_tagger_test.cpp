...
int main(int argc, char** argv) {
  ...
  if (!seq_prefix) {
    LOG(fatal) << "[sequence] group must contain a prefix to store model files"
               << ENDLG;
    return 1;
  }
  ...
  std::vector<sequence::sequence> testing;
  {
    ...
    for (auto i = static_cast<uint8_t>(begin); i <= end; ++i) {
      ...
      for (uint8_t j = 0; j <= *section_size; ++j) {
        ...
        for (auto& seq : sequences) testing.emplace_back(std::move(seq));
      }
    }
  }
  ...
  classify::confusion_matrix matrix;
  {
    ...
    for (uint64_t i = 0; i < testing.size(); ++i) {
      ...
      for (const auto& obs : seq) correct.emplace_back(obs.tag());

      tagger.tag(seq);

      for (uint64_t t = 0; t < seq.size(); ++t)
        matrix.add(predicted_label{seq[t].tag()}, class_label{correct[t]});
    }
  }
  matrix.print_stats();
