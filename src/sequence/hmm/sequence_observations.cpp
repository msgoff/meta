...
auto sequence_observations::expected_counts_type::operator+=(
...
sequence_observations::sequence_observations(expected_counts_type&& counts)
    : models_{[&]() {
        ...
        for (auto& ec : counts.counts_) models.emplace_back(std::move(ec));
        return models;
      }()} {
  // nothing
}
...
auto sequence_observations::expected_counts() const -> expected_counts_type {
  return {num_states(), models_.front().num_states(), models_.front().prior()};
}

uint64_t sequence_observations::num_states() const { return models_.size(); }
