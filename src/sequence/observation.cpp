...
const symbol_t& observation::symbol() const { return symbol_; }
...
void observation::symbol(symbol_t sym) { symbol_ = std::move(sym); }

void observation::tag(tag_t t) { tag_ = std::move(t); }

void observation::label(label_id lbl) { label_ = std::move(lbl); }

bool observation::tagged() const { return static_cast<bool>(tag_); }
