...
perceptron::perceptron() : analyzer_{default_pos_analyzer()} {
  ...
  analyzer_.add_observation_function([](const sequence& seq, uint64_t t,
    ...
    coll.add("q[t-1]w[t]=" + prev + "-" + utf::foldcase(seq[t].symbol()), 1);
  });
}
  ...
  for (uint64_t epoch = 1; epoch <= options.max_iterations; ++epoch) {
    ...
    auto time = common::time([&]() {
      printing::progress progress{
          " > Iteration " + std::to_string(epoch) + ": ", sequences.size()};
